...
void showPOND(POND *head,float *wave,float *wind){
	...
	int _cut=MAX_PATTERN;
	if(_cut>num-1)_cut=num-1;
	int __cut=num-MAX_PATTERN;
	if(__cut<0)__cut=0;
	float cutoff1=score[_cut];
	if(cutoff1<0.0f)cutoff1=0.0f;
	float cutoff2=score[__cut];
	if(cutoff2>0.0f)cutoff2=0.0f;
	i=0;
        while(head){
		if(head->score>=cutoff1 || head->score<=cutoff2){
			printf("--- %d ---\n",i++);
...
int reduce(int *arr,int len,float ratio){
        char *flag=(char*)calloc(len,sizeof(char));
        int l=(int)(((float)len)*ratio);
        int ll=0,i=0;
        float _l=(float)len;
        while(l--){
                i=(int)(_l*frand());
...
POND *mergePOND(float **MA,int XA,int YA,float **MB,int XB,int YB,float *wave,float *wind,POND *PA,POND *PB){
	...
	p=reduce(pos,p,REDUCE_RATIO);
	float *scaled_wave=(float*)malloc(p*sizeof(float));
	for(i=0;i<p;i++){
		center+=(float)pos[i];
		scaled_wave[i]=wave[pos[i]];
	}
	center/=(float)p;
	scale(scaled_wave,p);
	pond->num=p;
...
POND *createPOND(float **MA,int XA,int YA,float **MB,int XB,int YB,float *wave,float *wind,int from,int to,int from2,int to2){
	...
	pond->center=((float)(to+to2+from+from2))/4.0f;
	pond->scaled_wave=scaled_wave;
	pond->score=score;
	pond->previous=NULL;
	pond->next=NULL;
...
float filterPOND2(POND **head,POND **tail){
	POND *here=NULL,*tmp=*head;
	int i=0,num=0;
	tmp=*head;
	while(tmp){
...
int stream(float **MA,int XA,int YA,float **MB,int XB,int YB,int width){
	...
	printf("\n");*/
	//float *aaa=(float*)malloc(235*sizeof(float));
	//for(j=0;j<YA;j++){
	//for(i=0;i<235;i++){
	//	if(i)printf(",");
		//printf("%f",MA[j][ind[i]]);
	...
	//}
	//printf("\n");
	//float score=getScore(MA,XA,YA,MB,XB,YB,ind,235,aaa,wind);
	//printf("score=%g\n",score);
	//exit(0);
